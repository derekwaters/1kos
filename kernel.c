...
void exit(int exitcode) {
	current_proc->exitcode = exitcode;
...
__attribute__((naked)) void switch_context(uint32_t *prev_sp,
	...
	__asm__ __volatile__(
		...
		"lw s10, 11 * 4(sp)\n"
		"lw s11, 12 * 4(sp)\n"
		"addi sp, sp, 13 * 4\n"	// 13 4-byte registers popped from the stack
		"ret\n"
	);
}

__attribute__((naked)) void user_entry(void) {
	// Set the CPU into user mode (RISC-V U-Mode)
	__asm__ __volatile__(
		"csrw sepc, %[sepc]				\n"			// supervisor set the program counter (where sret jumps to)
...
struct process *create_process(const void *image, size_t image_size) {
	...
	*--sp = (uint32_t) user_entry;	// ra
	...
	for (uint32_t off = 0; off < image_size; off += PAGE_SIZE) {
		...
		// Handle the case where remaining data to be copied is smaller
		// than the page size.
...
void yield(void) {
	...
	// If we can't find another runnable process, let's continue
...
void kernel_main(void) {
	...
	yield();	// Cede control to the first available process
...
void boot(void) {
	__asm__ __volatile__(
		...
		"j kernel_main\n"	// Jump to the kernal main function
		:
